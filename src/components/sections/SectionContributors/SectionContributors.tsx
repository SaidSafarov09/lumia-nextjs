...
export const SectionContributors = () => {
  ...
  return (
    <section className={styles.sectionBlock} id='block'>
      <motion.div className={styles.grid} ref={ref}
      ...
      {data.map((item, index) => {
        return (
          <div key={index}  >
            ...
            {item.hasSecondLogo && (<div className={styles.logos}><img src={item.logo} alt="Logo" />
            <img src={item.logoTwo} />
            </div>)}
            {!item.hasSecondLogo && (
              <div className={styles.onlyOneLogo}><img src={item.logo} alt="Logo" />
              </div>
            )}
          </div>
        );
